...
firmware_load() {
  ...
  echo "Arduino: Excutable created: $ELF_INSTALL_PATH" > /dev/kmsg
}
...
firmware_stop() {
  # Stop the firmware
  echo "Arduino: Stoping $ELF_INSTALL_PATH" > /dev/kmsg
...
case "$1" in
  ...
  *)
    ...
    echo "    This command must be excuted while the script contains the binary"
